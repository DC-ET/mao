...
搭建/环境/部署：README.md、DEPLOY.md。产品/使用问答：skills/mao-cli/SKILL.md。细案：docs/plan/（2026-05-20-technical-design.md、2026-08-04-android-app-technical-design.md）。初版：重构不考虑存量数据与兼容。
...
## 测试
`cd backend-ts && npm test`；`cd agent-cli && npm test`；根 `npm test` Playwright（`tests/playwright.config.ts`，`tests/{admin,desktop}.spec.ts`，勿依赖固定用例数）。E2E 首次先 `bash scripts/e2e-setup.sh`（建 mao_e2e 库+迁移+种子+生成 backend-ts/.env.e2e，勿提交）；global-setup（tests/global-setup.ts）自动拉起隔离后端 :9180 + 两个 dev server，端口占用则复用；勿连线上 9080；运行期目录（runtime/users/workspace/uploads）经 .env.e2e 重定向到 backend-ts/.e2e-data/，严禁触生产 /opt/mao-data。E2E：admin 用 `login()`（admin/admin123）；settings-cards/integration 需 `MAO_ADMIN_PASS=admin123`；桌面未认证弹登录；主题用 `page.addInitScript` 设 localStorage（Vite 下 `evaluate` 不可靠）；断言 `toContainText`/`toBeVisible`；分页默认第一页。
