...
- `core-logic-audit-20260917-1902.md`：B01 子代理重试 untrack 窗口、B02 WS 重连 Promise 丢失、B03 待办/队列写错会话、B04 历史分页混文件变更、B05 聚焦列表未纳入新建会话、B06 角色权限保存覆盖、B07 重试结果 2000 字截断。
- `logic-bugs-core-features-20260910.md`：insert_message 不校验 PENDING、定时任务 QUEUED 永不完结等。
...
**历史区别**：`code_review_20260902205050.md` 记录的是 CLI 无 BOM/CRLF/二进制探测等能力缺口；**并行 RMW 竞态**是新增逻辑洞，与能力清单无关。
...
**历史区别**：`logic-bugs-review-20260824.md` 讲的是无 max-duration 时 error 帧导致无限等；本条是 **已有 max-duration 时的武装时机与作用域错误**。
...
**历史区别**：`logic-bugs-report.md` 提到过 read 路径 splitLines 问题；本条明确落在 **FileChangeDiffUtil 与 CLI computeLineDelta 的跨端不一致**，且 CLI 注释已声称同口径但实现未对齐。
...
5. 文件名 `logic-bugs-core-features-20260917-1146.md` 创建前已确认与 `docs/code-review/` 既有文件不重名。
