...
- 审查范围：`backend-ts/src` 全量走读 + 定向复核。基线：commit `d4b8982` + 当前工作区未提交改动（即 `2026-09-01-code-review-01.md` 所审查的 6 项修复已计入现状）。
...
- **对照**：`2026-08-07-code-review-17.md` 记录并修复过 **delegate 子会话**追问路径的同类问题（落库前清理尾部孤立 USER）；微信入站路径无对应处理。
...
- **位置**：`backend-ts/src/session/ws/streaming-ws-handler.ts:748`（add）、`:764`（cleanupIncompleteTail）、`:769`（updatePhase RESUMING）；无 try/catch 保护。对比：`submitExecution` 的 catch（`:368-372`）只兜底「提交被拒」；WS dispatch 外层 catch 仅 `console.error`（`2026-09-01-code-review-01.md` 亦确认该 catch 静默吞错）。
...
- **后果**：claim 永久残留，此后该会话所有 send / retry / 队列消费全部命中 `session_already_running`（`:135`、`:744`、`:982` 等互斥检查），只能重启进程恢复。与 M-4（`2026-08-30-logic-bug-review-01.md`，仅覆盖 `handleSendMessage` 的 saveMessage/prepareMessage 路径，现 `:297-338` 已修）及 `2026-08-24-logic-bug-review-02.md:275`（`handleCreateSideSession` 的 agentExecutor 拒绝，`:648` 路径已修）互不重叠——retry 路径的**前置 await 异常**是同一根因的第三处漏改。
...
- **去重说明**：`2026-08-31-bug-claims-01.md:27` 论证的是「listDue 排除 SENDING + eventKey 唯一键」下单实例不会重复投递，未覆盖本条「WAITING_WS 到期被 claim」的竞态；`2026-08-24-logic-bug-review-02.md` BUG-13（无条件覆盖终态、已修）是另一问题。
...
- **对照已修**：`2026-08-23-logic-bug-review-01.md:201` 记录的「回退 walk 缺 node_modules/.git 跳过」现已通过 `IGNORED_DIRS`（`:190`）修复，与本条无关。
...
2. **`McpSyncService.nameToId` 全局 Map 无用户隔离/无清理**：已被 `2026-08-21-code-review-06.md:86` 记录，不可重复报告。
...
- 明确排除的既有记录：`2026-08-30-logic-bug-review-01.md`（M-4 handleSendMessage claim、L-9、M-5 定时任务 T0 快照等）、`2026-08-31-bug-claims-01.md`（schedule 侧 withSessionLock 等 6 条，其中 ④⑤ 已在当前工作区修复）、`2026-08-24-logic-bug-review-02.md`（BUG-13 resolveWebSocket 覆盖终态、:275 handleCreateSideSession claim、BUG-34 等）、`2026-08-07-code-review-17.md`（delegate 孤立 USER）、`2026-08-21-code-review-06.md:86`（nameToId）、`2026-09-01-code-review-01.md`（今日修复审查，确认 schedule 侧同型锁 bug 已修而微信侧未修）。
