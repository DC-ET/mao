...
**历史区别**：旧报告 `2026-08-16-code-review-02.md` 提到正常子代理完成后父循环只检查 `hasRunning` 的问题；当前循环已增加 `hasPendingResults`，本条是 **重试收尾在入队前提前 untrack** 的独立缺口，现有双检查仍无法覆盖。
...
**历史区别**：区别于 `2026-09-01-code-review-04.md` 的 TCP 黑洞无 onclose 超时，也区别于主动 disconnect 未清 Promise；这里 onclose 已到达、下一次连接也已成功。
...
**历史区别**：历史 `2026-09-01-frontend-review-02.md` 的 A13 是未保存时切换角色丢失修改；本条无需切换角色，是保存期间编辑被在途响应覆盖。
