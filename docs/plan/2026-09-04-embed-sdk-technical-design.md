...
- 对话浮窗：Shadow DOM 挂载，含消息流（用户气泡 / 助手正文平铺、Markdown 渲染、思考过程折叠、工具调用状态卡片）、文本输入框、发送、**手动停止执行**、"新对话"按钮、错误与重连提示。
- 视觉语言对齐 Mao 桌面端（`desktop/src/style.css`）：默认主色 `#0066cc`，发丝级描边 + 分层柔和阴影 + 8/10/14/16px 圆角阶梯。`theme.primary` 覆写后 hover 色、浅底色、聚焦环由 `color-mix` 按主色重算（不支持 `color-mix` 的浏览器落回静态默认值）。动效遵循 `prefers-reduced-motion`。
