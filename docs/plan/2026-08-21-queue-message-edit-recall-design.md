...
消息队列功能上线后（见 `2026-06-05-message-queue-design.md`），Agent 执行中用户发送的消息会进入队列等待自动消费。当前队列消息仅支持上移/下移/删除/立即发送，不支持编辑——用户发现已入队的消息写错了，只能删除后凭记忆重新输入，带图片的消息还要重新上传。

会话内最后一条用户消息的编辑能力已由 `edit_and_resend` 实现（`2026-06-05-feature-edit-user-message.md`），但它要求 Agent 空闲，对队列中的消息完全不适用。
