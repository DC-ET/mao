...
const HELP = `用法:
  mao session list [--keyword] [--status]
  mao session groups [--keyword] [--status] [--preview-limit]
  mao session list --group-key <key> [--keyword] [--status] [--offset] [--limit]
  mao session get --id <id>
  mao session create --agent-id <id> --execution-mode LOCAL|CLOUD [选项...]
  mao session update --id <id> [--title] [--summary] [--project-key] [--permission-level] [--model-id]
  mao session delete --id <id>
  mao session mark-read --id <id>
  mao session pin --id <id>
  mao session favorite --id <id>
  mao session archive --id <id>
  mao session dashboard
