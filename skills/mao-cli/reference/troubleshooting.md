...
| 管理后台重新部署后，已打开的页面里部分菜单点击无反应 | 标签页仍在请求部署前的旧脚本。0.0.180 起会自动整页打开所点菜单；仍无反应时强制刷新一次 |
...
- ECP 飞书登录用户突然提示「无有效 ECP 票」、须重新绑定：旧版若在自动续期途中重启后端，该用户 `renew_status` 会卡在 `RENEWING` 且不再被续期扫描命中，票到期即失效。0.0.177 起超过 5 分钟未收敛的 `RENEWING` 每分钟复位一次，重新进入续期队列
...
- 工具执行后返回 `400`，字段指向 `messages.*.tool_calls.*.summary`：旧版把内部工具摘要传给了不接受该字段的模型渠道。0.0.111 修复为在 Chat Completions 请求序列化时排除工具调用的 `summary`（流式/非流式均生效），内部摘要保留；升级后端后可继续使用原会话，无需清空历史或修改渠道参数。
- `LLM API returned 400` 且 `No tool output found for tool call ...`：历史里 assistant 的某个 tool_call 没有对应 tool 结果（并行工具只落了一部分、或工具图片插在两次 tool 结果中间把配对拆开）。旧版会让该会话无法继续也无法重试。0.0.144 起会补占位 tool 结果并把图片挪到整组 tool 输出之后；升级后端后可在原会话点重试，不必新开。
- 飞书失败卡没有「重试」按钮 / 点了提示「无法定位原进度卡片」：进度卡映射只在 FAILED 终态保留（供重试定位 bot）。0.0.144 起失败卡带「重试」；升级前已失败的旧卡若映射已被清掉，需重新发送消息触发新任务。
- 同一会话从 Responses 协议切到 Chat Completions 后 400（`messages[*]: missing field \`type\``）：Responses 流式历史 tool_calls 可能缺 `type`。0.0.128 起流式写入与 Chat Completions 序列化均补齐为 `function`；升级后端后原会话可继续使用。
- 执行过程突然只剩一个工具调用、刷新后又出现：旧版会在模型流重试时清空整条流式气泡。0.0.111 改为只丢掉本次未完成的生成，已完成步骤保留；未升级前端时可刷新页面从历史恢复。
- 执行异常原样保留模型错误，Agent 不再追加 `LLM call failed:`。若显示 `LLM API returned 400`，应根据后面的字段路径与错误详情排查请求参数；修复重复前缀不会解决上游参数校验失败。
...
- 常驻会话（`keep_session`）里不要用 `exit`/`exec` 结束循环，那会杀掉整个 bash；应使用 `break`。若命令已经 `exit`，工具会把剩余输出标为 `completed:true` 并关闭会话，无需再 `await_async`（0.0.177 起 `write_stdin` 写 `exit` 也会真正回收会话，不再占用会话数配额）
- `async: true` 的后台任务跑过 30 分钟也能拿到结果：0.0.177 起超时只告警、结果继续保留交付；旧版会把任务移出登记表，`await_async` 只返回「后台任务不存在或已被消费」且结果永久丢失。仅超过 24 小时未结束才回收
...
- 状态 `QUEUED`：会话忙，已排队
- 排队消息在对话里出现两条一模一样的内容：旧版自动消费时若执行提交被拒（并发打满），会回补队首却不删已落库的消息，下次消费再写一条。0.0.177 起统一补偿（回补队首 + 删孤儿消息），不再重复或丢失
...
- 换渠道须重新填 Webhook
- 收到「执行失败」推送但对话页任务是「已完成」且看不到失败痕迹：后端重启（蓝绿部署）后崩溃恢复对同一会话并发续跑了两次，失败的那一路抢先落了 FAILED 终态并发出通知，成功的一路随后把会话改回 COMPLETED。0.0.109 已修复；升级前遇到以对话页最终结果为准
- 所有人突然都收不到通知（Webhook 自测正常）：旧版调度器读后台参数失败一次（DB 抖动等）就断了轮询链，日志只有「任务通知调度参数读取失败，1 分钟后重试」，实际不会重试，须重启后端。0.0.177 起失败后 1 分钟自动重排
...
| 手机上输入符号时重复输出前面已输入的内容 | xterm 隐藏输入框累积值被输入法整段重发（[xterm.js#6078](https://github.com/xtermjs/xterm.js/issues/6078)）。0.0.98 起触屏端自动清扫；仍出现时按一次回车或 Ctrl+C 可清空累积值 |
