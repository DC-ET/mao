...
「设置 → 飞书Bot」绑定飞书账号：发起授权 → 在打开的飞书页面完成授权（Electron 内嵌窗口 / 浏览器新页）→ 自动轮询绑定结果。绑定以飞书 `union_id` 为身份锚，绑定一次对所有机器人通用。

绑定后可在飞书内与机器人对话：私聊直接发消息，群里 @ 机器人触发（自动注入最近群聊上下文）。未绑定时收到引导文案。解绑后机器人无法识别身份，原工作区与会话保留。
