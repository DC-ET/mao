...
运行中可停止；停止后可继续发消息。会话空闲时点停止只做一次「待取消」登记，不会把从未运行过的任务标成「已取消」（0.0.177 起）。断线时点工具审批的「批准 / 拒绝」会提示发送失败并把审批项放回队列，可恢复网络后重试，不会出现审批消失而任务仍在等待的假死。Side Task 可并行子会话；子智能体由 `delegate` 工具创建，在检查器中查看。边路任务点停止、边路/子代理点重试时，若网络不可用会提示失败并保持原按钮可用，不会把停止入口关掉或把重试按钮卡死。
