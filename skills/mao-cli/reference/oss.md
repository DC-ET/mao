...
```bash
mao oss sts-token --session-id 12 --json
mao oss sts-token
```

---

## 命令：mao upload-config get
...
- OSS 与上传配置 0.0.82 起在管理后台「系统设置 → 集成配置」维护（凭证 AES 加密、支持试签测试），环境变量仅在升级首次启动时自动导入
