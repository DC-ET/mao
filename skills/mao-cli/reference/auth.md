...
用户名密码登录，成功后写入 `~/.mao/auth.json`（只存 JWT，不存密码）。
...
```bash
mao auth login --username demo --password 'secret'
```
...
```bash
mao auth feishu-qrcode --json
```

---

## 命令：mao auth feishu-status
...
```bash
mao auth feishu-status --state abc123
```

---

## 命令：mao auth feishu-callback
...
```bash
mao auth feishu-callback --code AUTH_CODE
```
