...
管理当前用户的个人 Skill（`/user-skills`），以及下载 LOCAL 模式技能同步包（`/skills/sync-package`，二进制 zip）。全局技能目录见 [skill-docs.md](skill-docs.md)。
...
- 隐藏文件（以 `.` 开头）不会上传
- 同名技能会被覆盖
- 上传校验（0.0.84 起，桌面端）：最多 500 个文件、单文件 ≤ 20MB、总量 ≤ 50MB；`node_modules` / `.git` / `dist` 等目录会被跳过并提示
...
```bash
mao skill delete --name my-skill
```

---

## 命令：mao skill sync-package
...
```bash
mao skill sync-package --session-id 12 --out /tmp/skills.zip
```
