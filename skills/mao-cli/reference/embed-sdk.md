...
- **自定义下拉**：Vue Element / Ant Design 等远程搜索框不是原生 `<select>`。`page_fill` 对 combobox 默认保持焦点并短暂等待建议（可用 `blur: true` 强制失焦）；出现选项后重新 `page_inspect`，再 `page_click` 对应 option。`page_select` 只操作原生 `select`。点中 option 后以 `observation.selected === true`（或重新 inspect 出现选中标签）为准；`multiple`/`allow-create` 下 fill 成功只代表过滤文本已写入，不代表 v-model 已绑定。
...
| 历史列表 | 标题栏「历史」入口展开当前 Agent 的会话列表（仅 SDK 浮窗创建、`source=embed` 的会话，最近活跃在前，20 条/页触底自动翻页），点击切换继续对话；切换不中断进行中任务，切回靠重拉历史 + 快照对账还原；流式输出中「新对话」与切换均可用。宿主可用实例方法 `toggleHistory()` 控制开合。升级前创建的常驻会话首次打开浮窗时自动补标来源并入列 |
...
| 附件 | 输入框支持直接粘贴剪切板里的图片或文件（无上传按钮）：粘贴后在输入框上方显示缩略图/文件 chip，可逐个移除，最多 10 个，超过后台配置 `file.maxSizeMb` 的文件当场提示并忽略。发送时图片上传到 Mao 服务并以多模态图片随消息发出，其他文件上传到会话 runtime 临时目录、以 `@{绝对路径}@` 引用写进正文；单个失败只提示失败文件名，成功的附件照常发出，全部失败且无正文时不发送，避免「只发出去文字」。气泡内图片显示缩略图、文件显示文件名 chip，刷新后从历史一并还原 |
...
| 推荐问题 | Agent 详情含 `suggestedQuestions` 时，新会话空白态在提示文案下方竖排展示（最多 5 条、按后台排序）；点击填入输入框并聚焦，由用户确认后手动发送，不自动发出。发送首条消息后隐藏，「新对话」后重新出现；未配置或拉取失败时不渲染该区块，与未上线该功能时一致 |
...
- **CSP**：放行 Mao 域名的 `connect-src`（`https:` 与 `wss:`）、`script-src` 及 `img-src`（Agent 头像、粘贴附件的图片缩略图；页面截图还需 `data:` 与 `blob:`）。
...
| 粘贴附件后发送提示「附件上传失败」 | 单个文件失败只影响该附件。确认宿主 CSP 放行 Mao 域名 `connect-src`（multipart POST 带预检）；文件超过后台「系统设置 → 集成配置 → 上传配置」的 `file.maxSizeMb` 会在输入区就被拦下，不会进入待发列表 |
| 粘贴图片后模型回「当前模型不支持图片输入」 | 该 Agent 配置的模型不支持视觉输入，换支持视觉的模型或在管理后台调整该 Agent 的模型 |
