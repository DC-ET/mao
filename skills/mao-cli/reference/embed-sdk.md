...
3. `claims.id` 作为固定身份键，公司需保证不可回收复用。首次登录按可信邮箱唯一匹配已有启用账号（含管理员），否则创建普通用户；禁用/删除账号、重复邮箱和已有绑定冲突拒绝自动关联。SSO 不授予、不撤销管理员角色。
...
| 执行过程 | 流式思考、正文与工具调用按事件顺序穿插，只有相邻同类节点合并；正文保持可见，思考与连续工具调用在执行中及结束后均默认折叠。工具摘要显示数量、执行中与失败状态，展开查看参数和结果；后续流事件不重置用户的展开选择。等待模型输出时用三点脉动（与桌面端一致），工具正在执行时不叠一层 loading。模型流中断自动重试时保留已完成的说明和工具调用，只丢掉本次未完成的生成 |
