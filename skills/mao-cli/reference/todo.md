...
```bash
mao todo list --session-id 12 --json
```
...
```bash
mao todo update --session-id 12 --todo-id 3 --status completed
mao todo update --session-id 12 --todo-id 3 --content '改写说明'
```
...
```bash
mao todo delete --session-id 12 --todo-id 3
```
