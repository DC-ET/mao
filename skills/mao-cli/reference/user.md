...
```bash
mao user list --page 1 --size 20 --keyword admin --status 1
```
...
```bash
mao user create --username alice --password 'Secret1' --display-name 爱丽丝 --role-ids 1,2 --status 1
```
...
```bash
mao user update --id 3 --display-name 新名字 --status 1
```

## 命令：user reset-password

为任意账号设置本地登录密码，不校验旧密码。LDAP、飞书、ECP、公司 SSO 账号设置后仍可用原方式登录，同时可用用户名和该密码登录。有本地密码后，该用户 `authSource` 为 `LOCAL`。密码须 8–64 位，且同时包含字母和数字。
...
```bash
mao user reset-password --id 3 --new-password 'NewPass1' --confirm-password 'NewPass1'
```
...
```bash
mao user set-status --id 3 --status 0
```
...
```bash
mao user assign-roles --id 3 --role-ids 1,2
```
