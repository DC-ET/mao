...
登录后默认进入本页，也是侧边栏 logo 的「返回首页」目标。页面按**分析维度拆 6 个 Tab**（总览 / 趋势 / 模型 / 用户 / Agent / 会话），壳层统一周期筛选（今日 / 昨日 / 3 / 7 / 30 / 90 天）、自动刷新（关闭 / 5s / 10s / 30s / 1m，默认关闭）与刷新；URL 同步 `?tab=&period=`，首屏只拉当前 Tab，切换周期只重拉已访问模块：
