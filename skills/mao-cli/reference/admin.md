...
- 结构：会话结局分布与模型 Token 占比环形图；Agent Token 与用户活跃横向排行
- 明细：模型用量表（会话 / 消息 / 对话与后台 Token / 占比）、窗口内失败会话（可跳会话详情）

90 天视图默认聚焦最近 30 天，可拖动查看全周期。实时异常（运行中、待审批、卡住会话）看运行监控，本页阶段分布只统计窗口内创建的会话。
