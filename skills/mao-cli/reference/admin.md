...
- **总览**：主指标 Token 消耗（窗口值 + 环比 + spark）+ 次指标（新增会话 / 消息 / 活跃用户，副文案分别展示完成·失败会话、后台调用、活跃口径）；运行态按「实时快照 / 窗口结果」分组；右侧为规则洞察 + 窗口构成（对话/后台 Token、完成/失败会话），次指标可下钻对应 Tab。页头以周期区间为主，环比配色说明收进 tooltip。
- **趋势**：会话 / 消息分小多图（取消双 y 轴）+ Token 堆叠柱；可切换调用次数/失败、成功率/缓存命中率视图
- **模型**：模型 Token 占比（Top 10 + 其他）+ 用量明细表（含成功率、缓存命中、延迟均值）；场景/协议分布；默认排除连通性测试，可勾选含自检
