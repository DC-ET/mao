...
只适用于部分用户的技能不要传到系统 Skills。在「个人 Skills」里选择目标用户后上传，效果与这些用户自己在桌面端上传相同：仅他们可用，并在其所有智能体中生效；同名个人技能会被覆盖。若与系统技能同名，这些用户使用这份个人技能。拖放会按松手时选中的用户上传，读取目录完成前不能切换页签。`node_modules`、`.git`、`.svn`、`dist`、`__MACOSX` 会跳过；最多 500 个文件、单文件 20MB、总量 50MB。手机端可查看和删除，上传请在电脑浏览器完成。列表读需 `skill:read`，上传与删除需 `skill:write`。
...
| 个人指令 | 各用户在桌面端创建的个人指令（`/admin/user-commands`）：按用户展示、关键词筛选、查看正文、删除。不在此新建或直接编辑；可将一条个人指令「添加到系统指令」，复制为全局指令（可改名称与正文），原个人指令保留 |

两 Tab 均支持关键词筛选与分页；指令内容列表默认三行截断，悬停或点「查看」看完整正文。查看需 `command:read`，新增、编辑、删除、提升为系统指令需 `command:write`。CLI：`mao system-command list|list-personal|...`（见 [command.md](command.md)）。

## 用户管理

新增/编辑用户、禁用、重置密码、分配角色。

列表「账号类型」列标注账号来源（后端 `authSource`）：本地 / LDAP / 飞书 / ECP / 公司 SSO，可按来源筛选。来源判定顺序：有本地密码 → 本地；否则看最早绑定的 `user_external_identity.provider` → ECP 或公司 SSO（同时绑定两者时取最早那条，只归入一个来源）；再看飞书绑定 → 飞书；其余为 LDAP。

管理员可对任意账号重置密码，写入的是本地登录密码。LDAP、飞书、ECP、公司 SSO 账号设置后仍可用原方式登录，同时可用用户名和该密码登录。一旦有本地密码，列表账号类型显示为「本地」，按原外部来源筛选时不再出现。
...
查看全站会话：用户、Agent、状态、消息与工具调用。查看需 `session:read`，归档和删除需 `session:write`。用户反馈异常时从此定位。

会话详情里，会话信息和聊天记录分开加载。聊天记录不带文件 diff 正文，工具输出和过长参数按页面能展示的长度截断，完整内容用「导出记录」。文件变更不插在每次写入中间：一轮结束后，在该轮回复末尾合并成一块；这一轮还在执行时先不展示，和客户端一致。

「任务阶段」在会话等待用户回答提问时显示为「等待回复」，与模型推理中的「运行中」区分开；用户详情页里的会话列表同样如此（0.0.195 起）。等待回复时聊天记录末尾会补一条进行中的 `ask_user_questions` 工具卡片（内存态伪消息，用户作答落库后由真实记录取代；0.0.199 起）。

CLI：`mao admin-session ...`（见 [admin-session.md](admin-session.md)）。

## 定时任务

查看 Cron、启用/暂停、删除任意用户任务（读需 `scheduled-task:read`，写他人任务需 `scheduled-task:write`）；支持关键词/用户/Agent/状态/完结筛选与分页；定位 `QUEUED`（会话忙时排队）与失败记录。
...
追踪登录失败、用户/角色变更、模型与 Agent 配置变更、敏感操作。进入本页需要 `audit:read`。可开启「自动刷新」（每分钟）；切到其他菜单再回来会按开关状态自动续上，不会出现开关仍开启但列表停止刷新的情况（0.0.178 起）。

CLI：`mao audit ...`（见 [audit.md](audit.md)）。

## 用量分析（管理员首页）

有 `analytics:read` 时登录后默认进入本页。没有该权限时进入自己有权限的第一个页面（会话、Agent、用户、系统设置、模型，按这个顺序），侧边栏 Logo 和「返回首页」也回到该页。页面按**分析维度拆 6 个 Tab**（总览 / 趋势 / 模型 / 用户 / Agent / 会话），壳层统一周期筛选（今日 / 昨日 / 3 / 7 / 30 / 90 天）、自动刷新（关闭 / 5s / 10s / 30s / 1m，默认关闭）与刷新；URL 同步 `?tab=&period=`，首屏只拉当前 Tab，切换周期只重拉已访问模块。切换周期 / 子 Tab 只更新地址栏，顶部标签栏始终只保留一个「用量分析」标签（0.0.178 起；此前每种筛选组合都会新增一个同名标签），点回该标签会还原上次的筛选条件：
