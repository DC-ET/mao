...
须先在管理后台「系统设置 → 集成配置 → 公司 SSO」启用并设置校验域名白名单（例如 `acg.team` 允许自身和全部子域）及宿主 Origin 白名单，保存后新换票即时生效，不再使用 SSO 环境变量。业务系统通过 `auth.checkUrl` 指定校验地址，域名信任范围由配置人员决定。官方校验成功后，按稳定身份绑定、可信邮箱关联已有普通账号或创建普通用户；不自动绑定管理员。SDK 在凭证临期前换票并在线更新 WS 认证，SSO 有效期间不限制连续使用时长。换发的 Token 与该 Mao 用户普通登录等权，不是仅能访问某个 Agent 的专用凭证。
