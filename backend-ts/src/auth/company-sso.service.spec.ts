...
describe('CompanySsoService', () => {
  it.each([3600, 120])('issues actual bounded TTL for %i remaining seconds and no refresh', async (remaining) => {
    ...
    const service = new CompanySsoService(config, { verify: vi.fn().mockResolvedValue(verified) }, { resolve: vi.fn().mockResolvedValue({ user: { id: 4, username: 'test' }, action: 'bound' }) }, jwt);
    const result = await service.exchange('synthetic', 'https://sgs.acg.team/check');
  ...
  it('passes each URL through the real client without changing the stable subject or provider', async () => {
    ...
    const service = new CompanySsoService(config, new CompanySsoClient(config, fetcher), { resolve }, jwt);
    for (const checkUrl of ['https://acg.team/business/check?app=first', 'https://sgs.acg.team/other/path?app=second']) {
      const result = await service.exchange('synthetic', checkUrl);
  ...
  it('rejects near expiry before writing accounts and rechecks after DB work', async () => {
    const resolve = vi.fn().mockResolvedValue({ user: { id: 4, username: 'test' }, action: 'existing' });
    const service = new CompanySsoService(config, { verify: vi.fn().mockResolvedValue(identity(29)) }, { resolve }, jwt);
    await expect(service.exchange('synthetic', 'https://sgs.acg.team/check')).rejects.toMatchObject({ code: 1402 });
    ...
    await expect(new CompanySsoService(config, { verify: vi.fn().mockResolvedValue(verified) }, { resolve }, jwt).exchange('synthetic', 'https://sgs.acg.team/check')).rejects.toMatchObject({ code: 1402 });
  });

  it('rate limits verified subject across exchanges', async () => {
    const service = new CompanySsoService(config, { verify: vi.fn().mockResolvedValue(identity(3600)) }, { resolve: vi.fn().mockResolvedValue({ user: { id: 4, username: 'test' }, action: 'existing' }) }, jwt);
    for (let i = 0; i < 20; i++) await service.exchange('synthetic', 'https://sgs.acg.team/check');
    await expect(service.exchange('synthetic', 'https://sgs.acg.team/check')).rejects.toMatchObject({ status: 429 });
  });
});
