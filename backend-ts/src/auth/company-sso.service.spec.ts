...
describe('CompanySsoService', () => {
  it.each([3600, 120])('issues actual bounded TTL for %i remaining seconds and no refresh', async (remaining) => {
    ...
    const result = await service.exchange('synthetic', 'https://sgs.example.com/check', config);
  ...
  it('passes each URL through the real client without changing the stable subject or provider', async () => {
    ...
    for (const checkUrl of ['https://example.com/business/check?app=first', 'https://sgs.example.com/other/path?app=second']) {
  ...
  it('rejects near expiry before writing accounts and rechecks after DB work', async () => {
    ...
    await expect(service.exchange('synthetic', 'https://sgs.example.com/check', config)).rejects.toMatchObject({ code: 1402 });
    ...
    await expect(new CompanySsoService({ verify: vi.fn().mockResolvedValue(verified) }, { resolve }, jwt).exchange('synthetic', 'https://sgs.example.com/check', config)).rejects.toMatchObject({ code: 1402 });
  });
  ...
  it('rate limits verified email across exchanges', async () => {
    ...
    for (let i = 0; i < 20; i++) await service.exchange('synthetic', 'https://sgs.example.com/check', config);
    await expect(service.exchange('synthetic', 'https://sgs.example.com/check', config)).rejects.toMatchObject({ status: 429 });
  });
});
