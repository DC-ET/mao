...
export const config: CompanySsoConfig = { enabled: true, allowedDomains: ['acg.team'], allowedOrigins: ['https://host.example.test'], accessTtlSeconds: 1800, timeoutMs: 3000, requireHttps: false };
...
describe('CompanySsoClient', () => {
  it('uses only trusted remote claims and fixed GET protocol', async () => {
    ...
    const identity = await new CompanySsoClient(config, fetcher).verify('synthetic-token', checkUrl);
  ...
  it.each(['https://acg.team/business/check?app=mao&scope=a%2Fb', 'https://child.sgs.acg.team:8443/other/check?app=two'])('passes the supplied business URL to GET: %s', async (url) => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(response({ code: 0, success: true, data: { illegal: false, claims: claims() } }));
    await new CompanySsoClient(config, fetcher).verify('synthetic+token/=', url);
  ...
  it.each(['https://evilacg.team/check', 'https://acg.team.evil.com/check', 'http://acg.team', 'https://acg.team?token=existing', 'not a URL'])('rejects %s before any outbound request', async (url) => {
    ...
    await expect(new CompanySsoClient(config, fetcher).verify('synthetic', url)).rejects.toMatchObject({ status: 400 });
  ...
  it.each([
    ...
    await expect(new CompanySsoClient(config, vi.fn().mockResolvedValue(response(body))).verify('synthetic', checkUrl)).rejects.toMatchObject({ status: 503 });
  });
  ...
  it('distinguishes explicit illegal and expired credentials', async () => {
    for (const data of [{ illegal: true }, { illegal: false, claims: { ...claims(), exp: 1 } }]) {
      await expect(new CompanySsoClient(config, vi.fn().mockResolvedValue(response({ code: 0, success: true, data }))).verify('synthetic', checkUrl)).rejects.toMatchObject({ status: 401 });
    }
  });
  ...
  it('bounds streamed responses and suppresses transport error details', async () => {
    for (const fetcher of [
      ...
      await expect(new CompanySsoClient(config, fetcher).verify('synthetic', checkUrl)).rejects.toMatchObject({ status: 503, message: 'SSO service is unavailable' });
    }
  });
  ...
  it('times out without exposing the request URL', async () => {
    ...
    await expect(new CompanySsoClient({ ...config, timeoutMs: 5 }, fetcher).verify('synthetic', checkUrl)).rejects.toMatchObject({ status: 503 });
  });
});
