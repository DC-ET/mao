...
export interface FeishuNormalizedMessage { eventId: string | null; messageId: string | null; chatId: string | null; chatType: FeishuChatType; senderId: string | null; senderUnionId: string | null; maoUserId?: number; senderType: string | null; messageType: string; imageKey?: string | null; fileKey?: string | null; fileName?: string | null; text: string; mentions: string[]; isBotMentioned: boolean; content: unknown; rawEvent: unknown; header?: FeishuEventHeader; }
...
export interface FeishuHarnessService { prepareMessage(sessionId: number, content: unknown): Promise<string> | string; execute(sessionId: number, eventId: string | null, listener: AgentEventListener, cancelFlag: CancelFlag): Promise<void>; }
